...
noop *xoa_cay(noop **root, char ten[])    
{
    if ((*root)==NULL) return (*root);
    else if (strcmp((*root)->usename,ten) > 0) xoa_cay((*root)->left, ten);
    else if (strcmp((*root)->usename,ten) < 0) xoa_cay((*root)->right, ten);
    else if (strcmp((*root)->usename,ten) == 0)
    {
    	printf("sinh vien bi xoa\n");
    	printf("ten dang nhap %s\n", (*root)->usename );
    	printf("password %s \n",(*root)->password );
    	printf("diem %f\n",(*root)->mark );
    	noop *temp = (*root);
        if ((*root)->left == NULL) { (*root)=(*root)->right; return (*root); }
        else if ((*root)->right == NULL) { (*root)=(*root)->left; return (*root); }
        else
        {
            noop *temp1 = (*root);
            noop *temp2 = temp1->left;
            while (temp2->right != NULL)
            {
                temp1 = temp2;
                temp2 = temp2->right;
            }
            strcpy(temp->usename,temp2->usename);
            strcpy(temp->password,temp2->password);
